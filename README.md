...
程序运行期间会监听文本剪贴板。按 `Ctrl+Alt+V` 随时唤出小窗；使用方向键选择后按 Enter，或按数字 `1–9` 快速复制。复制完成后默认自动隐藏。复制多段内容时可按 `Ctrl` 或 `Shift` 多选，再点击 **复制**。点击标题栏的 `?` 或按 `F1` 可随时查看应用内快捷键说明。点击 **Expand / 展开** 可进入完整管理界面；再次点击 **Compact / 悬浮** 回到小窗。
...
- v0.3：可选 DOI 元数据、系统密钥保护、图片/文件历史和导入策略。
